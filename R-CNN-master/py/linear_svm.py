    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            print('{} - positive_num: {} - negative_num: {} - data size: {}'.format(
                phase, data_set.get_positive_num(), data_set.get_negative_num(), data_sizes[phase])) # po:625  ne:625 total:1250

            # Iterate over data.
            for inputs, labels, cache_dicts in data_loaders[phase]:
                inputs = inputs.to(device)  # 128    cache_dicts框的位置，个数128
        ...
        train_dataset = data_loaders['train'].dataset  # 训练集
        remain_negative_list = data_loaders['remain']    # 负样本
        jpeg_images = train_dataset.get_jpeg_images()  # 训练集样本图片
        transform = train_dataset.get_transform()  # 图片处理操作

        with torch.set_grad_enabled(False):
            remain_dataset = CustomHardNegativeMiningDataset(remain_negative_list, jpeg_images, transform=transform)
            # 为了让模型正常训练，我们必须要通过某种方法抑制大量的简单负例，挖掘所有难例的信息
            # 难负例挖掘：多找一些hard negative加入负样本集，进行训练
...
if __name__ == '__main__':
    ...
    data_loaders, data_sizes = load_data('./data/classifier_car')  # 加载数据集
    ...
    model.classifier[6] = nn.Linear(num_features, num_classes)   # 修改网络结构，最后输出为2
    model.load_state_dict(torch.load(model_path))  # state_dict变量存放训练过程中需要学习的权重和偏执系数
    model.eval()   # 模型用于验证
    # 固定特征提取
