...
def nms(rect_list, score_list):
    ...
    rect_array = np.array(rect_list)  # 正样本候选区域框位置
    score_array = np.array(score_list)  # 正样本候选区域框得分
...
if __name__ == '__main__':
    ...
    start = time.time()
    for rect in rects:   # 遍历所有候选区域框
        xmin, ymin, xmax, ymax = rect
        rect_img = img[ymin:ymax, xmin:xmax]

        rect_transform = transform(rect_img).to(device)  # 图片处理
        output = model(rect_transform.unsqueeze(0))[0]  #  利用模型计算特征（汽车）

        if torch.argmax(output).item() == 1:   # 输出最大值的索引为1时，是正样本（汽车），0为负样本（不是汽车）
            """
            预测为汽车
            """
            probs = torch.softmax(output, dim=0).cpu().numpy()    # 正样本的概率值

            # tmp_score_list.append(probs[1])
            # tmp_positive_list.append(rect)

            if probs[1] >= svm_thresh:  # 通过阈值（>0.6）过滤正样本候选建议
                score_list.append(probs[1])  #  添加符合条件的得分
                positive_list.append(rect)  #  添加正样本候选建议框位置
